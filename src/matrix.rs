...
impl<F: PrimeField> Matrix<F> {
  ...
  fn gauss_jordan_general(&mut self) {
    ...
    for r in 0..row_count {
      ...
      if self[(r, lead)] != F::zero() {
        let div = self[(r, lead)];
        for j in 0..col_count {
          self[(r, j)] = self[(r, j)] * div.invert().unwrap();
        }
      }
...
impl<F: PrimeField> Mul<Poly<F>> for &Matrix<F> {
  type Output = Poly<F>;
...
impl<F: PrimeField> Add<Matrix<F>> for Matrix<F> {
  type Output = Matrix<F>;
...
impl<F: PrimeField> Into<Vec<F>> for Matrix<F> {
  fn into(self) -> Vec<F> {
    self.v
  }
}
